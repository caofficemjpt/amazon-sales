...
export function ProductsPage() {
  ...
  return (
    <div className="products-page">
      ...
      <div className="card">
        ...
        {advertisingData.length > 0 ? (
          ...
          <div className="empty-state" style={{ padding: 'var(--space-8)' }}>
            ...
            <p className="empty-state-title">No advertising cost data available</p>
          </div>
        )}
      </div>
      ...
      {/* New vs Repeat SKUs placeholder */}
      <div className="card products-placeholder-card">
        <div className="placeholder-icon"><RefreshCw size={24} /></div>
        <div>
          <h4 className="placeholder-title">New vs Repeat SKUs</h4>
...
function computeFeeData(fees: SettlementFee[]) {
  ...
  for (const fee of fees) {
    const sku = fee.sku ?? 'Unknown';
    const amount = fee.fee_amount ?? 0;
    bySkuMap.set(sku, (bySkuMap.get(sku) ?? 0) + amount);

    // Group fee types (combine similar ones)
...
function computeAdvertisingData(fees: SettlementFee[]) {
  ...
  for (const fee of fees) {
    ...
    const sku = fee.sku ?? 'Unknown';
    bySkuMap.set(sku, (bySkuMap.get(sku) ?? 0) + (fee.fee_amount ?? 0));
  }
